...
// 半径が 6 ではなく 8 なのは、リングを通り抜けられる最大の体幅が変わるため。
// 柱の隙間は Math.round() の丸めで一様でなく、斜めの経路も使えるので、
// 実測でリングを越えられる最大幅は半径 6 で 23px、半径 8 で 39px
// （level-generator.test.ts「闘技場: リングの帯を判定 2 倍の幅で通り抜けられる」）。
// ボスの当たり判定は 14px で、狙わずに通り抜けるにはその 2 倍（28px）の
// 幅が要るため、23px の半径 6 では足りず、39px の半径 8 なら足りる。
// これが効くのは「ボスがリングの外へ出るため」ではない（目標半径の上限 70px は
// 柱の外縁 76px より内側なので、ボスは外へ出ない）。半径 6 だとボスは座席から
// 43px 前後で頭打ちになり、目標半径の帯（10〜70px）の上端に一度も出られない —
// 帯の上半分を使わせるために半径 8 が要る（docs/gameplay.md「ボス階」）
