...
ボス（45）とその弾（46）は `tools/boss_tiles.py` が焼く。46 も絵ではなく単色の点なので `slash_tiles.py` と同じくコードが原本になり、色は既存の敵の赤 `(255,66,0)`（蜘蛛とセントリーの目と同じ色）を芯だけ橙に寄せたもので、どちらもフラグメントシェーダの full-bright 規則（`r>0.95 && g>0.25 && b==0`）を満たす。**この規則から外れる色に変えてはならない** — ライトを積めない弾を等しい明るさで見せる唯一の経路である（docs/enemies.md「ボス（灰皿撤去ユニット）」）。45 は `tools/atlas.py` が画像から焼くタイルで、いまは同じ `boss_tiles.py` が焼く暫定のプレースホルダが入っている。番号は tool と `game.ts`（本体の生成）/ `entity-boss.ts`（弾の生成）が共有する契約である。
