...
`#c` / `#h` / `#a` / `#hud` の 4 つを除き、画面全体を覆う UI はすべて明示的な `z-index` を持つ。`#hud` は `pointer-events: none` の透明な土台（`source/hud.css`）で、クリックを奪う心配がなく、ゲームプレイ中は他の全画面 UI より常に奥にあればよいだけなので、auto（実質最下層）のままにしてある。値の並びがそのまま手前後ろの並びで、値が近いほど関係が深い（同じ「押収品を開ける系」のダイアログは同値）: 死亡画面 `#ds` = 10、装備画面・ボス報酬ダイアログ = 12（同値。両者が同時に開くことはない）、OP `#op` = 15、ボスのフェーズ移行フラッシュ `#bf` = 18、一撃必殺の閃光 `#sl` = 19、白フェード `#wf` = 20。新しい全画面 UI を足すときは、この並びのどこに挟むかを先に決める。
