...
クリック待ちのあいだに `opening_preload()` が OP（`source/opening.ts`）の DOM 構築と素材先読みを済ませる。OP は予告編話法の 5 カット＋タイトルドロップで、進行表は `opening-model.ts`（葉モジュール）が持つ。音は最初のクリック後にしか鳴らせないため、OP 本編は必ずクリックの後に置く。スキップ（クリック、または M と修飾キー単独を除くキー）と完走は単一の `opening_finish()` に集約し、タイマー・音・動画をまとめて止める。OP の音は新しい instrument を作らず、既存 sfx のレート変更（`audio_play_op`）で賄う。動画（op5 / title）はループさせず 1 回再生して終端 pause ― ループは継ぎ目のフレーム差が隣接フレームの約 10 倍あり目視できる。動画に音声トラックは無く、背面タブでは `play()` が省電力停止されるため、自動検証はシークとデコードで行う。

OP を抜けると初回も死亡画面（自席の端末）を経由する。操作説明（移動・射撃・音声）は OP に置かず、この画面の下中央（`.ds-controls`）に常設する。

### フルスクリーン UI の重なりは z-index が正本

`#c` / `#h` / `#a` / `#hud` の 4 つを除き、画面全体を覆う UI はすべて明示的な `z-index` を持つ。`#hud` は `pointer-events: none` の透明な土台（`source/hud.css`）で、クリックを奪う心配がなく、ゲームプレイ中は明示的な `z-index` を持つ UI より常に奥にあればよいだけなので、auto のままにしてある。ただし auto は最下層という意味ではない ― `#hud` は `position: fixed` で `hud.ts` が body の末尾に後付けするため、DOM 順で `#c` / `#h` / `#a` より手前に描かれる。下の並びの最下段であって、画面の最下層ではない。値の並びがそのまま手前後ろの並びで、値が近いほど関係が深い（同じ「押収品を開ける系」のダイアログは同値）: 死亡画面 `#ds` = 10、装備画面・ボス報酬ダイアログ = 12（同値。両者が同時に開くことはない）、OP `#op` = 15、ボスのフェーズ移行フラッシュ `#bf` = 18、一撃必殺の閃光 `#sl` = 19、白フェード `#wf` = 20。新しい全画面 UI を足すときは、この並びのどこに挟むかを先に決める。
