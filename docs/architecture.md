...
### `prefers-reduced-motion` は CSS だけで畳む

`reduce` の環境では、アニメーションとトランジションの尺・遅延をほぼ 0 へ潰して最終状態に即座に置く。**JS 側に分岐を持たない**（`index.html` が `#wf` / `#sl` / `#bf` で採っている流儀と同じ）。`matchMedia` を読んで振る舞いを変える経路を作ると、「動きを減らす」という 1 つの判断が CSS と JS の 2 か所に分かれる。
