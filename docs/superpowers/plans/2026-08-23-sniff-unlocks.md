...
Expected: PASS。テスト数は 254 から 255 に増える（`it('コストは…')` 1 件が 3 件になり、合計コストの 1 件は置き換えなので ±0）。`typecheck` はエラーなしで終了する。
...
Lv4 の優先順位は「残り香 > 非常口」だが、実装に分岐はない。目標リストが空になるのは本物を吸い終えたときだけ（フロアには本物が必ず 1 つあるので、全部 `_done` ⟹ 一服済み ⟹ `exit_open`）なので、`exit_open` のとき非常口を目標に足すだけで、残り香が残っている間は BFS 距離の比較で残り香が勝つ。Lv4 が埋めるのは「一服後に非常口を探して歩き回り、ゲージが再び 60% を割った」局面で、それまでは目標リストが空になって嗅覚が沈黙していた。
...
```bash
grep -rn "10450\|Lv10\|10 段\|0.3 / 9\|3.33pt" docs/ README.md
```

Expected: `10450` / `Lv10` / `0.3 / 9` / `3.33pt` は 1 件も残らない。`10 段` は肺活量・耐性・脚力・火力を指す記述としてのみ残る（嗅覚を含む列挙が残っていないことを目で確認する）。

- [ ] **Step 10: コミット**

```bash
git add docs/meta-progression.md docs/gameplay.md docs/architecture.md docs/story.md README.md
