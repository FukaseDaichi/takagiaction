...
- Task 1〜13 は完了済み。Task 13 の Step 7（この plan と spec の削除）だけは、この Task が残っているあいだ保留してある。終わったら Step 7 を実行する。
- `m/q2.png` のタイル 45 には `tools/boss_tiles.py` の `bake_boss_placeholder()` が焼いた暫定の絵（灰色の機体＋`(255,66,0)` の目 8px）が入っている。**下の Step 1 の `tools/atlas.py` は、それを本物の絵で上書きする作業**である。
- この plan と spec の記述は `docs/` とコードに劣後する（`AGENTS.md`）。実際に食い違っているのは 3 点: Task 13 Step 5 の確定文「45 は `tools/atlas.py` が焼く画像」（現物は上記のプレースホルダ）と「full-bright 規則を満たす唯一の帯」（アンバー `(255,190,0)` も満たす。`docs/enemies.md`「清掃ドローン」）、Task 5 の「隅は外周の輪郭壁から 2 タイル内側」（実装は一番外側の床から 2 タイル内側）、闘技場の頂点数「床 497 + 壁 128」（実測は床 495 + 壁 130 で 6090 頂点）。正しいのは `docs/architecture.md`「アトラスの焼き込み」・`docs/gameplay.md`「ボス階」とコードのコメントである。
